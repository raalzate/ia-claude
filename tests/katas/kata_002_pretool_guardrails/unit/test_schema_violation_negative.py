"""TS-012: negative amount → schema_violation (positive-decimal invariant)."""
...
def test_negative_amount_rejects_as_schema_violation(
    policy_snapshotter: Path,
    session_tmpdir: Path,
    read_jsonl,
) -> None:
    payload = json.loads((FIXTURES_DIR / "negative_amount.json").read_text())
