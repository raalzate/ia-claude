...
- [ ] T045 Write `katas/kata_010_prefix_caching/README.md` (Principle VIII deliverable): objective, prompt-caching architecture (two-region composer + `cache_control` placement), cache-hit measurement via `response.usage.cache_read_input_tokens`, anti-pattern defense (AST lint + runtime digest detector), run instructions (offline fixtures + `LIVE_API=1` live probe), reflection prompts, and explicit documentation of `cache_control` placement (ephemeral on static only), TTL expectations (5-minute ephemeral window), and the API's breakpoint budget (<= 4 cache breakpoints per request).
