...
- [ ] T066 Write `katas/017_batch_processing/README.md` covering: objective (route latency-tolerant workloads through Message Batches API), Message Batches API architecture (classifier → submitter → batches API → mapper/bucket/cost), anti-pattern defense (no synchronous loops over async-tolerant corpora; no silent drops), run instructions (offline fixtures + `LIVE_API=1` dry-run), reflection on Principle VIII, and an explicit section documenting the batch lifecycle (`submitted → in_progress → ended | canceled | expired`) plus partial-failure handling (isolate → fragment → bounded re-submit → stitch).
