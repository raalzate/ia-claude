...
- [ ] T036 Write kata `katas/013_headless_ci_review/README.md` with the six mandated sections: (1) **Objective** — deterministic reviewer per PR; (2) **Headless CLI architecture** — sequence diagram or textual walkthrough GitHub Actions → `claude -p --output-format json --json-schema` → validator → mapper → Checks API → artifact upload; (3) **CI integration** — example `.github/workflows/ci-review.yml` snippet and `gh pr checks` verification; (4) **Anti-pattern defense** — why free-form/regex parsing is forbidden and how the AST lint enforces it; (5) **Run** — install, local dry-run (`python -m katas.013_headless_ci_review.runner`), `pytest tests/katas/013_headless_ci_review -v`; (6) **Reflection** (Principle VIII) — prompt-regression tests, prompt/glue decoupling. Document the **headless invocation contract** as a table: flag → guarantee (`-p` → no TTY; `--output-format json` → parseable; `--json-schema` → schema-bound). Document **exit codes** table: `0` success or schema-valid empty; `2` schema_failure; `3` cli_failure; `4` mapping_failure.
...
- [ ] T040 [P] Cross-link `README.md`, `quickstart.md`, `spec.md`, and `plan.md` (relative links) so a practitioner entering at any doc can reach the others.
