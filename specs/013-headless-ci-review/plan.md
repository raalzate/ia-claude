...
| VIII. Mandatory Documentation (NN) | Every non-trivial function in `mapper.py`, the workflow YAML, and each schema will carry a *why* comment tied to the kata's anti-pattern defense. A single `notebook.ipynb` produced at `/iikit-07-implement` is the Principle VIII deliverable: it explains the kata, results, every Claude architecture concept exercised, the architecture walkthrough, applied patterns + principles, practitioner recommendations, the contract details, run cells, and the reflection. No separate README.md. |
