...
| VIII. Mandatory Documentation (NN) | Every non-trivial class (`ChainStage`, `Chain`, concrete stages, `StageBudgetExceeded`) will carry a *why* comment tied to the saturation anti-pattern. A single `notebook.ipynb` produced at `/iikit-07-implement` is the Principle VIII deliverable: it explains the kata, results, every Claude architecture concept exercised, the architecture walkthrough, applied patterns + principles, practitioner recommendations, the contract details, run cells, and the reflection. No separate README.md. |
