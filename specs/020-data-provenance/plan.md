...
| VIII. Mandatory Documentation (NN) | Every non-trivial function (the `Claim` validator, the tool-schema mirror, the canonical-claim-key hasher, the conflict detector, the orphan-claim lint) will carry a *why* comment tied to Principle VII or to the spec's anti-patterns. Feature ships a `README.md` covering objective, walkthrough, anti-pattern defense, run instructions, and reflection, written during `/iikit-07-implement`. |
