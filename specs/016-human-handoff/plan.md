...
| VIII. Mandatory Documentation (NN) | A single `notebook.ipynb` produced at `/iikit-07-implement` is the Principle VIII deliverable: it explains the kata, results, every Claude architecture concept exercised, the architecture walkthrough, applied patterns + principles, practitioner recommendations, the contract details, run cells, and the reflection. No separate `README.md`. *Why*-comments on every non-trivial function are deliverables of `/iikit-07-implement`. Traces to FR-012. |
...
| FR-012 docs describe preconditions + schema + anti-pattern | `notebook.ipynb` (Principle VIII deliverable) + *why*-comments |
