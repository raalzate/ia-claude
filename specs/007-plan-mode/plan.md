...
| VIII. Mandatory Documentation (NN) | Each non-trivial function carries a *why* comment tied to the kata objective (plan-mode gating) or the anti-pattern (jumping straight to edits on unfamiliar code). Ships `specs/007-plan-mode/README.md` during `/iikit-07-implement` covering objective, walkthrough, anti-pattern defense, run instructions, reflection. |
