...
- [ ] T056 Write `katas/018_scratchpad_persistence/README.md` (Principle VIII deliverable) covering: (a) **objective** — durable externalisation of investigation findings; (b) **scratchpad schema + persistence boundary** — fixed sections `map`/`findings`/`open_questions`/`decisions`/`conflicts`, YAML frontmatter + markdown body, `ScratchpadWriter` as the only write path; (c) **anti-pattern defense** — explicit "no context-window hoarding": findings never live only in conversational memory, and the compact-resume probe proves the round trip; (d) **run** — quickstart and `LIVE_API=1` instructions; (e) **reflection** — Principle VIII prompts on what was lost before the pad and where `conflicts` earned its keep
- [ ] T057 [P] Document the scratchpad read/write contract + compaction rules in `katas/018_scratchpad_persistence/README.md`: every write is flock-serialised, atomic via tmpfile + `os.replace`, followed by a parse-round-trip check; `/compact` at ≥ 0.55 fill emits a `ContextAnchor`; rotation at `MAX_SCRATCHPAD_BYTES = 100_000` archives to `<name>.<iso-date>.md`; session start re-reads the pad before the first user query
...
- Final Polish (T056–T063) depends on all prior phases; T062 and T063 are the acceptance gate.
