...
| VIII. Mandatory Documentation (NN) | Every non-trivial function carries a *why* comment tied to the kata objective (dual-total audit, silent-overwrite defense). A `README.md` with objective / walkthrough / anti-pattern defense / run instructions / reflection is written during `/iikit-07-implement`. |
