...
| VIII. Mandatory Documentation (NN) | Every non-trivial function (coordinator spawn path, subagent run, payload builder, validation gate) will carry a *why* comment tied to Principle IV or to the leak-probe anti-pattern. A single `notebook.ipynb` produced at `/iikit-07-implement` is the Principle VIII deliverable: it explains the kata, results, every Claude architecture concept exercised, the architecture walkthrough, applied patterns + principles, practitioner recommendations, the contract details, run cells, and the reflection. No separate `README.md`. |
