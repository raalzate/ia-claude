...
| VIII. Mandatory Documentation (NN) | Every non-trivial function / hook / schema will carry a *why* comment tied to FR-XXX / SC-XXX / anti-pattern. Feature ships a `README.md` covering objective, walkthrough (baseline vs. normalized diff), anti-pattern defense, run instructions, reflection — written during `/iikit-07-implement`. |
